...
function PlantDetailScreen({ route, navigation }) {
  ...
  useEffect(() => {
    fetch('https://openfarm.cc/api/v1/crops/?filter=' + name)
      .then((response) => response.json())
      .then((json) => {setDataOpenFarm(json);})
      .catch((error) => console.error(error))
      .finally(() => {setLoadingOpenFarm(false);});
  },[]);
   ...
   flex: 1
       ...
       {(isLoading && isLoadingOpenFarm) ? <ActivityIndicator/> : (
        <View>
          ...
          <Cultivator cultivator={data.cultivator} />
          <Variety variety={data.variety} />
          <TaxonomySection label="Species" name={data.species.name} />
          <TaxonomySection label="Genus" name={data.genus.name} />
          <TaxonomySection label="Family" name={data.family.name} />
          <TaxonomySection label="Order" name={data.order.name} />
          <TaxonomySection label="Class" name={data.plant_class.name} />
          <TaxonomySection label="Division" name={data.division.name} />
          <TaxonomySection label="Kingdom" name={data.kingdom.name} />
        </View>
      )}
      <Text></Text>
      <Text style={styles.bold}>Description:</Text>
      {isLoadingOpenFarm ? <ActivityIndicator/> : (
        <ScrollView>
        <View>
          <Text>{dataOpenFarm.data[0].attributes.description}</Text>
          <Text>Thanks to Open Farm Data</Text>
        </View>
        </ScrollView>
      )}
  </View>
  ...
  return (
    <View>
      <Text style={{fontSize: 30}}>
        <Text style={{fontStyle: 'italic'}}>{props.plant.genus.name} {props.plant.species.name.toLowerCase()} </Text>
        <VarietyHeader variety={props.plant.variety} />
        <CultivatorHeader cultivator={props.plant.cultivator} />
      </Text>
      <Text style={{fontSize: 20}}>
          {props.plant.common_names.map((name, index) => {
            var commas = "";
